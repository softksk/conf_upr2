"""
Этап 4: Дополнительные операции (порядок загрузки)
...
def main():
    """Главная функция - Этап 4"""
    if len(sys.argv) != 2:
        print("Использование: python stage4_main.py <config.ini>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("✓ Порядок загрузки определен")
        print()
